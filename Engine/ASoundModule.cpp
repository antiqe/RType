...
namespace Engine
{
  ...
  void		ASoundModule::_changeVolume()
  {
	...
	(*it).first->volume((((this->_percent[percent])[(((*it).first)->getType())]
			      * this->_percent[3][(((*it).first)->getType())] / 1.0) / static_cast<float>(nb)));
      }
  }
