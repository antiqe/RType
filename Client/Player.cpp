...
void	Player::update()
{
	...
	Engine::APowerComponent* power = dynamic_cast<Engine::APowerComponent*>(obj->getComponent(Engine::AComponent::POWER));
	this->_stats[Player::POWER]->setPercent(power->getPower());
	this->_stats[Player::ARMOR]->setPercent(armor->getArmor());
	this->_stats[Player::SPEED]->setPercent(speed->getSpeed() * 20);
}
